...
/**
 * Inline rich-text renderer for spec-authored copy (field descriptions,
 * conditions, note text). It is a small, synchronous, zero-dependency tokenizer
 * for the INLINE subset of markdown — which is all this data ever uses:
 *
 ...
 *
 * Why not <MDC> / a full markdown engine: this copy is always inline (verified
 * against the spec: no block quotes, lists, or headings ever appear), and <MDC>
 * is async (useAsyncData per instance) and caused SSR→client hydration
 * mismatches when rendered hundreds of times on one page. A synchronous
 * tokenizer is both lighter and SSR-stable. If block-level markdown ever

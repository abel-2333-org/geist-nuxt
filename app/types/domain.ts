 ...
 * `info`/`secondary` were added so preset wrappers that need a distinct hue
 * per value (e.g. HTTP methods: GET=info, PATCH=secondary) can still map onto
 * the atom instead of bypassing it. Mapping a domain concept (lifecycle, HTTP
 * method…) to one of these tones is a *preset* concern, kept out of the atom so
 * the atom is reusable.
 */

...
} from '~~/kits/api-docs/components/RelationSourcePath.vue'

// Structured operation relations —— 已通过真实 consumer 验收的正式 Gallery recipe。
//
// 背景：#28 的第一轮真实 consumer adapter validation 曾判定「semantics
// verified, presentation not accepted」。follow-up 把 reader-facing hierarchy
// 补全后，presentation 已通过真实 consumer 人工验收并解锁 #74 晋升。
//
// 本页用中性 fixtures 固化任务导向的整页装配。页面 markup、业务关系模型与
// fixtures 继续由 Gallery / consumer 持有；只把已稳定的来源路径与导航契约晋升
// 为 <RelationSourcePath>，不把整段 recipe 冻结成高阶 public API。
//
// 三条落地裁决：
//
...
// 页面最下方保留的「演示一次接力」是**补充**形态，不是默认主层级：它展示具体
// 值很直观，但表达不了层级，读屏上也弱。

definePageMeta({ nav: { label: '结构化关系', icon: 'i-lucide-git-branch', order: 6 } })
 ...
 *
 * 注：Gallery recipe 这版手写代码面。CodeBlock 刻意不带运行时高亮（只接 build-time
...
<template>
  <UContainer class="py-10 sm:py-14">
    <header class="mb-8 space-y-2 border-b border-default pb-6">
      <h1 class="text-2xl font-semibold tracking-tight text-highlighted">
        Structured operation relations
      </h1>
      <p class="max-w-3xl text-sm leading-relaxed text-muted">
        正式 Gallery recipe：callback 使用 Address Source，next operation 使用 Parameter / Value Source；来源以完整字段层级为主，原始 Runtime Expression 留在 compiled contract，不进入 Geist display model。页面 composition 继续由 consumer 持有。
      </p>
    </header>
      ...
      <section data-anchor-targets class="space-y-4 border-t border-default pt-8">
        <div class="space-y-1.5">
          ...
          <p class="max-w-2xl text-sm leading-relaxed text-muted">
            这些字段属于正式 Gallery 验收 harness。点击上方 Address Source 或 Parameter / Value Source

...
- `aria-expanded` / focus 归还由 reka Popover 内置——确认而非重写。
- **键盘打开时焦点移入面板**:浮层 portal 渲染,不在触发器之后的 DOM 顺序里;键盘打开(触发器 `:focus-visible`)后壳把焦点移到面板首个动作(无动作则面板本体,`tabindex="-1"`),Esc 归还触发器。指针打开保持 focus-neutral,hover 永不抢焦点。
- 同页字段跳转 `goTo(path, { focus: true })` 把焦点落到目标行;`initFromHash` 同样落焦,并 watch `route.hash` 覆盖动态路由复用实例的跨页到达。
...
- `useFieldSource` / `FieldAnnotation` / demo 目前经 `../../kits/api-docs/components/FieldItem.vue` 相对路径取 `FieldNode` 类型。晋升进 kit 时改为 `../components/FieldItem.vue`(source 与 registry copy-in target 拓扑一致);若要更稳,把 `FieldNode` 抽到 `kits/api-docs/utils/field.ts` 并让 FieldItem re-export 保持兼容,再让 composable / 组件从 util 导入。

...
// CANDIDATE (issue #117) — <PlaygroundFieldRow>.
//
// A short-lived fork of `kits/api-docs/components/FieldItem.vue`, the path
// `references/method/component-reflow.md` prescribes for evolving a kit
// component ("候选 FieldItem 先放 playground → 采纳后移动到 kits"). Everything
// below the marked DELTA comments is the kit component verbatim; the deltas are
// the whole proposal, so a reviewer can read the diff instead of the file.
//
// DELTA 1  `value` — the field's value shape (array element / record member /
//          encoded content), rendered by <PlaygroundFieldValueStructure>.
// DELTA 2  WITHDRAWN after review. A value-bearing field used to hoist its own
//          constraints into the value block under an auto scope heading
//          ("String requirements"). The consumer's real endpoint showed the
//          heading asserting something false: `retailers`' own rules are about
//          marketplace semantics, not about the wire string carrying them.
//          Author-supplied note labels (Rule / Consistency) were already
//          accurate, so the field's constraints stay in the field's own band,
//          exactly where the kit puts them. Scope headings survive only INSIDE
//          the value block, where they separate levels that really do differ.
// DELTA 3  descendant anchors include the value chain, so a deep link into an
//          element property still reveals its ancestors.
//
// Child count, anchor semantics, requiredness derivation, the amber fill
// ladder and the band order are untouched: the value block adds a channel, it
// does not renegotiate the row.
import {
  collectCandidateFieldPaths,
  collectValuePaths,
} from '../model/field-value'
import type { FieldValueLabels, FieldValueNode, ValueBearingField } from '../model/field-value'
import type { FieldItemLabels, FieldNode } from '../../kits/api-docs/utils/field'

interface PlaygroundFieldRowProps extends FieldNode {
  labels?: FieldItemLabels & FieldValueLabels
  /** DELTA 1 — the shape of this field's VALUE. Never a field itself. */
...
defineOptions({ name: 'PlaygroundFieldRow' })
...
const props = withDefaults(
  defineProps<PlaygroundFieldRowProps>(),
  {
    required: false,
    labels: () => ({}),
  },
...
// DELTA 3 — a value root and the real fields inside it are reachable anchors,
// so they belong in the set that decides "is the active anchor below me".
const childPaths = computed(() => [
  ...collectCandidateFieldPaths(props.children ?? []),
...
// DELTA 2 withdrawn — the field's own constraints stay in the band below,
// under the author's own note labels, exactly as the kit renders them.
...
// Read off `value.type` — data the model already carries, so no variant needs
// the consumer to supply anything new.
 ...
 * crosses an encoding boundary this is `codec<shape>` — the decoded shape is
 * the fact a reader scans a long field list for, and the wire type (`string`
 * for every encoded field) is not. Fields without a boundary keep `format`.
 */
...
<template>
  ...
  >
      ...
      >
        <div data-field-identity class="flex min-w-0 flex-wrap items-baseline gap-x-2 gap-y-1">
          ...
          <span
            ...
            :class="decodedShape && formatToken !== format ? 'text-dimmed' : 'text-muted'"
            translate="no"
          >{{ type }}</span>
          <!-- Variant B/C give this token the type's own weight, not the
               dimmed treatment: when every wire type is `string`, the decoded
               shape is the discriminating fact and the faded one is `string`. -->
          <span
            v-if="formatToken"
            class="wrap-anywhere font-mono text-xs"
            :class="decodedShape && formatToken !== format ? 'text-muted' : 'text-dimmed'"
            translate="no"
          >{{ formatToken }}</span>
          <span
    ...
    <!-- DELTA 1 — value requirements & structure. NOT children: nothing in this
         block is a named field, so nothing in it is counted by the disclosure
         below, anchored as a field, or reachable as `children`. -->
    <PlaygroundFieldValueStructure
      v-if="value"
      :value="value"
      :labels="labels"
      :show-verb="t.showChildren"
      :hide-verb="t.hideChildren"
    />

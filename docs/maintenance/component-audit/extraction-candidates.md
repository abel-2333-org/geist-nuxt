...
- **2026-09-19 每日反向横扫**（base `52e7f1d`，簇 `api-docs-sidebar-nav` / `api-docs-code-rail`）:无新增候选。`touch-manipulation` 触控契约在分发面共 8 处（kit：FieldItem ×3、FieldValueStructure、SchemaComposition、OperationTarget、SidebarScenarioTags `+N` 触发器；foundation：AnnotationPopover 触发器；SidebarNav 的分区 header 按钮为 09-11 记录的缺口，复审后连同搜索清除按钮、行链接一并补齐；FieldItem 的 `?` 记号按钮触屏不开 tooltip 不计），单一 utility class、无共享逻辑，不是候选；stretched-link（`absolute inset-0` 的 `ULink` 覆盖层 + `pointer-events-none` 兄弟内容层）全仓仅 SidebarNav 一处，FieldItem / FieldValueStructure 的 `absolute inset-0` 是 09-15 已评估的 arrival-cue 装饰 overlay，语义不同；隐藏测量层（`invisible absolute -z-10` + `data-tag` / `data-plus` 样本 + `document.fonts.ready` 重测）仅 SidebarScenarioTags 一处；`MutationObserver` + `ResizeObserver` 双观察者的增量目标同步（`syncPair`）仅 CodeRail 一处；`computeSplitBudgets` 内容优先重分配纯函数全仓唯一（ADR-010 刻意留在 kit）；全局 `'/'` 快捷键的 `window.addEventListener('keydown')`（含 typing / IME 守卫）仅 SidebarNav 一处，SiteSearch 的 `⌘K` 走 Nuxt UI `defineShortcuts`，两者键语义与守卫条件不同（见 `reports/2026-09-19.md`「为什么没改」），不抽；`isComposing` 守卫、ASCII `slug()` 回退全仓各仅一处；`router.resolve` 在 SiteSearch（导航目标归一化）与 SidebarNav（active 推断）用途不同；受控 `UCollapsible` open map 与 `role="status"` live region 家族分别在 09-11 / Round 2 已裁定不抽，无新调用点。
- **2026-09-20 每日反向横扫**（base `d126cfe`，簇 `api-docs-site-search` / `api-docs-operation-header` / `api-docs-lifecycle-badge`）:无新增候选。三项 owning file 自上次实质改动（09-05 / 09-05 / 08-01）以来零变更，特征模式与既有结论逐一比对:`preset[props.status]` 封闭联合查表（LifecycleBadge / LifecycleNotice / HttpMethodBadge 带 fallback / SemanticBadge `BADGE_TONE_COLOR`）09-03 已判天然免疫、09-08 已判抽 composable 净收益为负；`.trim() ?` 空白回落一行式 3 处（LifecycleBadge / LifecycleNotice / ResponseExample）09-08 已裁定不抽；`headingLevel?: 2 | 3 | 4` + `` `h${…}` `` 家族约定、`defineOptions({ inheritAttrs: false })` + 多根、`tabindex=-1` + rAF focus + reduced-motion、`router.resolve`、`role="status"` live region、`ml-auto flex shrink-0` 行尾动作位与 `text-balance` 标题均为 09-05 / 09-19 已评估维度，无新调用点；`watch` + `setTimeout` + `onCleanup` 的 debounce / 陈旧取消套路全仓仅 SiteSearch 一处（AnnotationPopover 的开合定时器是 hover 意图延迟、`useCopy` 的是反馈复位，语义不同，不是候选）；`Fuse` 实例化仅 SiteSearch 一处（`utils/field.ts` 只是注释提及）；`items-baseline` 徽章 / 标识首行对齐在 OperationHeader 与 FieldItem identity 行（`data-field-identity`）各一处，属 class 级排版约定；三个 preset wrapper 的 `size?: BadgeProps['size']` 默认 `sm` 是 08-01 裁定的显式对齐记录。`touch-manipulation` 触控契约（SidebarNav 源码注释自述为「kit 的 tap 目标触控契约」）本日补到 SiteSearch 触发按钮，属契约一致性修复而非抽取候选；分发面其余 8 个组件的 `UButton`（CopyButton / CodeBlock / ResponseExample / WebhookProtocol / FieldAnnotation / DocAnnotation / TermAnnotation，以及 AnnotationPopover 的 `#error` retry 按钮）尚未纳入，见 `reports/2026-09-20.md` 遗留。
- **2026-09-21 每日反向横扫**（base `d126cfe`，簇与 09-20 相同:`api-docs-site-search` / `api-docs-operation-header` / `api-docs-lifecycle-badge`）:`origin/main` 未前进、计划与 09-20 完全一致，本日为 PR #149 的复核日；三项 owning file 与 09-20 审计终态（`014c1c8`）同 blob（`SiteSearch.vue` 相对 base main 含本 PR 的触控契约修复，另两项与 base main 同 blob），特征模式与命中集合无变化，09-20 的逐项结论原样成立，无新候选、无待决策候选。

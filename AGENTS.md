...
> 本文件是 **Codex / OpenAI 代理**的入口。它只是**指针**，正文全部在 `references/**`，
> 请按需打开对应文件，不要凭记忆臆造组件、token 或 API。

## 权威来源

构建**任何 UI**（页面、布局、表单、导航、反馈、覆盖层、主题）时，
**geist-nuxt 是本项目唯一权威的设计系统来源**。视觉参考 Vercel Geist，
...
- **领域包（仅当项目属于该场景时加载）**：`references/kits/api-docs/index.md`
  - 组件源码在 `assets/kits/api-docs/`，用到时装入应用的 `app/` 目录
...
- 用系统资源，不用占位图。
- 领域组件（如 API 文档那套）默认**不加载**，只有做该场景时才从 `assets/kits/` 装入。

## 起步

新项目从 `assets/starter/` 起步——provider、全局样式、字体已接好，
在其上加路由和组件即可，不要重建脚手架。

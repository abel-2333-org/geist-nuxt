...
for (const theme of ['light', 'dark'] as const) for (const width of [1440, 390]) {
  ...
  test(`${theme}/${width}: Sidebar and SiteSearch real method badge states`, () => scenario(theme, width, 'navigation', async (page, capture, evidence) => {
    ...
    const destinations = await badges.evaluateAll(elements => elements.map(element => element.closest('li')?.querySelector(':scope > a')?.getAttribute('href')))
    expect(destinations.every(destination => typeof destination === 'string' && destination.length > 0)).toBe(true)
    expect(new Set(destinations).size, 'each actual method row has its own destination').toBe(destinations.length)
    for (const destination of destinations) {
      const rowLink = nav.locator(`li > a[href=${JSON.stringify(destination)}]`).filter({ visible: true })
      expect(await rowLink.count(), 'the actual method destination remains available').toBe(1)
      await rowLink.click()
      await expect.poll(() => new URL(page.url()).pathname + new URL(page.url()).hash).toBe(destination)
      nav = await openNavigation()
      const activeRow = nav.locator('li').filter({ has: page.locator(':scope > a[aria-current="page"]') })
      const activeLink = activeRow.locator(':scope > a[aria-current="page"]')
      expect(await activeLink.getAttribute('href')).toBe(destination)
      const activeBadges = activeRow.locator(methodBadge).filter({ hasText: methodNames, visible: true })
      expect(await activeBadges.count()).toBe(1)
      await page.mouse.move(0, 0); await settle(nav)
      await capture(activeBadges, { component: 'SidebarNav', state: 'route-active', destination })
    }

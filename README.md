...
- **Codex / Claude Code**：Codex 读取根 `AGENTS.md`，Claude 通过 `CLAUDE.md` 导入同一规则；两端从项目内 skill 入口按需加载根 `SKILL.md` 与 references。入口只链接真源文件，不维护副本。消费项目用 `geist:skill` 安装项目内 skill，用 registry 工具安装 runtime 资产，不复制整套 gallery。

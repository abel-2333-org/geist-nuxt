...
//
// Endpoints may be named by use case rather than REST path: not every API is
// strictly RESTful, and one endpoint can serve several usage scenarios
// (basic, asynchronous, batch, …). So an endpoint appears ONCE. Its single
 ...
 *  a leading `method` badge + a purpose-name `label` (e.g. "Create resource")
 *  + the usage `scenarios` it serves as trailing tags. */
export interface SidebarNavItem {
  /** Visible label (already localized). For endpoints this is the purpose
   *  name, NOT necessarily the path — purpose names often make clearer menu labels. */
  label: string
...
// so "批处理" (a scenario) and "POST" (a method) both narrow the tree.

...
// Recomputing synchronously inside the ResizeObserver callback can resize the
// observed node within the same delivery cycle ("ResizeObserver loop completed
// with undelivered notifications"); useRafTask defers to the next frame and
// coalesces bursts of resize events into a single recompute.
const { schedule: scheduleRecompute } = useRafTask(recompute)

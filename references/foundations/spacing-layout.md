...
- **子树结构线只有一个来源**：内容从属于上方一行时用 foundation `main.css` 的 `subtree` utility（1px `--ui-border` 起始边 + 随具名容器 `field` 宽度收紧的缩进），不手写 `border-s border-default ps-*`，也不与 `ps-*` / `border-s*` 并写（tailwind-merge 不识别两者冲突，后者会在所有宽度胜出）；分组标题、事实列、语义 rule 不画结构线。规则与允许的线数见 `references/kits/api-docs/index.md`「折叠与层级语法」。

...
> 活的间距展示见 `FoundationsSection.vue` 的 spacing scale。

## 容器与页面宽度

 - **主容器用 `UContainer`**：它读取 `--ui-container`（starter 设为 **1200px**，Geist 内容列宽），并自带响应式左右 padding（`px-4 sm:px-6 lg:px-8`）。不要用裸 `<div class="max-w-...">` 手搓容器。
- **正文阅读宽度**：长文本/段落限制在 `max-w-2xl`（≈42rem）～`max-w-3xl`，避免过长行长——见 hero 段落的 `max-w-2xl`。
- **全宽区块**（如 banner、footer）可跳出 `UContainer`，但内部内容仍用容器对齐。
...
- 间距/圆角展示：`assets/starter/app/components/sections/FoundationsSection.vue`
- 节奏用法：`assets/starter/app/components/sections/HeroSection.vue`、`ComponentsSection.vue`
- 容器/圆角 token：`assets/starter/app/assets/css/main.css`（`--ui-container`、`--ui-radius`）

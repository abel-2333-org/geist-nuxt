...
app/
  ...
  components/             # 自动导入的组件（starter 默认 pathPrefix: false，无目录前缀）
    ...
    sections/HeroSection.vue  # → <HeroSection />（子目录仅用于组织，不进组件名）
...
- **你自己 `app/components/**` 下的组件**也自动导入。**starter 默认关闭目录前缀**（`nuxt.config.ts` 里 `components: [{ path: '~/components', pathPrefix: false }]`）：`app/components/sections/HeroSection.vue` 在模板里就是 `<HeroSection />`，`app/components/api/CodeBlock.vue` 是 `<CodeBlock />`。子目录只用于组织源码，不进组件名。**代价**：所有组件 basename 必须唯一，否则解析冲突。（若某项目确实需要目录前缀，把 `pathPrefix` 改回 `true` 即可，此时名字变回 `<SectionsHeroSection />`。）
...
- **组件找不到 / 解析为空**：starter 默认 `pathPrefix: false`，用 basename 短名（`<HeroSection />`、`<CodeBlock />`），不要带目录前缀（`<SectionsHeroSection />` 反而找不到）。另检查是否有两个同名 basename 冲突。

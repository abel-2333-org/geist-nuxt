...
Nuxt UI 组件消费 Tailwind 的 `--shadow-*` token：卡片用 `shadow-xs`，**所有浮层**（popover / menu / select / toast / modal / slideover）默认都用 `shadow-lg`。starter 在 `main.css` 重映射这些 token，并通过 `app.config.ts` 把 modal/slideover 单独指向更重的 `shadow-xl`：
...
- **单 token 约束**：Nuxt UI 用同一个 `shadow-lg` 服务 popover 和 modal。要让 modal 更重，starter 已在 `app.config.ts` 用 `ui.modal.slots.content: 'shadow-xl'`（tailwind-merge 去重后 `shadow-xl` 胜出）。新增需要重阴影的浮层组件时，用同样方式指向 `shadow-xl`。
...
- **不要**长时间、循环、抢注意力的动画。
- **必须**尊重 `prefers-reduced-motion`——starter 的 `main.css` 已全局处理（reduce 时动画/过渡收敛到近瞬时）。

### easing 与时长

starter 把 Geist 的招牌弹性曲线设为 Tailwind 默认 `--ease-out`，故 `transition`/`ease-out` 工具类和 Nuxt UI 自身过渡都采用它：

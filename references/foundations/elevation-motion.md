...
Nuxt UI 组件消费 Tailwind 的 `--shadow-*` token：卡片用 `shadow-xs`，**所有浮层**（popover / menu / select / toast / modal / slideover）默认都用 `shadow-lg`。foundation 在 `geist.css` 重映射这些 token，并通过 `foundation/config/` 把 modal/slideover 单独指向更重的 `shadow-xl`：
...
- **不要**长时间、循环、抢注意力的动画。
- **必须**尊重 `prefers-reduced-motion`——foundation 的 `geist.css` 已全局处理（reduce 时动画/过渡收敛到近瞬时）。
...
- token 定义：`foundation/assets/css/geist.css`（`@theme` 的 `--shadow-*`/`--ease-out`、`:root`/`.dark` 的 `--shadow-xs`、末尾 reduced-motion 块）

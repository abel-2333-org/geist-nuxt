...
- 需要 JS 判断断点时用 VueUse 的 `useBreakpoints`（Nuxt UI 已依赖 VueUse），不要自己监听 resize。

## 源码参考

 - 容器主题：`src/theme/container.ts`（reference workspace: nuxt/ui@v4）
 - starter 用法：`assets/starter/app/components/sections/HeroSection.vue`（`UContainer`）、`ComponentsSection.vue`（响应式 grid）

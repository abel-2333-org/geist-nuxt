...
视口断点（`sm:`/`lg:`…）回答「屏幕多大」，但**可变宽的组件**（可拖拽的侧栏、放进不同栏宽的卡片、宽度不定的面板）真正在乎的是「**我自己**多宽」——这跟视口未必相关。这类场景用 **Tailwind v4 容器查询**，不要用视口前缀去猜。

- 在尺寸边界上标 `@container`，容器内的元素用 `@sm:` / `@min-[15rem]:` 等**容器前缀**（相对容器宽度，非视口）。
...
- 范例见 `kits/api-docs/sidebar-nav.md` 的场景标签簇：滚动区挂 `@container`，每行据侧栏实时宽度在「首标签 + `+N`」与「计数 chip」间切换，和拖拽调宽形成正反馈。

> 何时用哪个：布局随**屏幕**变 → 视口断点；组件随**自身容器**变（尤其宽度可被用户/父级改变）→ 容器查询。

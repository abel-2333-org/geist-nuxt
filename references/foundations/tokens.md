...
| `warning` | amber | **Geist amber 阶** | Geist 用琥珀而非黄色 |
...
值取自 **Geist 官方 `geist-colors` 包**（green / blue / amber / red / teal 的亮色阶），与 primary 同款机制注册成 `--ui-color-{role}-{shade}`，并设 `--ui-{role}` 浅色取 `500`、深色取 `400`。
...
```
success #45a557  info #0072f5  warning #ffb224  error #e5484d  secondary #12a594
```

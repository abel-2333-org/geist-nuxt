...
- **fallback 只派生不回写**：绑定值为未知 id、场景列表变化后失配时，展示确定性收敛到第一个场景，但不修改绑定值、不发事件——无更新循环，SSR 安全。uncontrolled 选择对原数组的原地增删同样遵循：已移除的选择被丢弃，之后重新加入同名项也不会复活旧选择（与 `<ResponseExample>` 行为一致）。

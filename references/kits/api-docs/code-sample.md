...
自包含多语言代码块——近单色、无语法高亮器。这是 API 文档代码块的**可复用基座**：`RequestExample` / `ResponseExample` 把 body 委托给它。

> 文件名是 `components/api/CodeBlock.vue`。starter 默认 `pathPrefix: false`，所以模板名就是 `<CodeBlock>`（不带 `Api` 目录前缀）。
...
- **复制委托给共享 `CopyButton`**：不在 CodeBlock 内重写剪贴板逻辑。CopyButton（`UButton` + `useCopy`）自带 copied 态图标切 check、动态 `aria-label`、`role=status` live region 播报，以及 iframe/insecure-context 的 execCommand 兜底 + toast。
...
<!-- 本地化 chrome 文案 -->
<CodeBlock :variants="variants" :labels="{ copy: '复制代码', copied: '已复制' }" />
```
...
- `<CopyButton>` — 通用复制按钮（基座组件），CodeBlock 的复制委托给它。

## 规格与源码

- 规格模板见 `method/component-spec-template.md`。
- 源码：`assets/kits/api-docs/components/CodeBlock.vue`、`composables/useCodeWrap.ts`；复制依赖 `assets/starter/app/components/CopyButton.vue` + `composables/useCopy.ts`。

...
| `components/FieldItem.vue` | `<FieldItem>` | 递归字段行：summary 按开发者扫描顺序拆为 signature（名/type/format/Required 或 Conditional/lifecycle）与 trailing fallback facts（默认值）两区，必填标记经 `fieldRequiredState({ required, condition })` 派生（`condition` 即蕴含 conditional，`required: true` 冲突时胜出）；`condition` 是 `ConditionText`（单句或有序句子列表），经 `conditionEntries()` 归一化：1 条与字符串展示完全等价，≥2 条在**同一条** rule 内由内部 `ConditionEntries` 渲染为 `<ul role="list">`，空列表与空白条目既不出 rule 也不产生标记；lifecycle 始终跟随字段身份簇换行，避免在窄容器形成孤立徽章行，按字段容器自身宽度而非 viewport 回流，长字段名与深层递归优先保住 identity 宽度；可选缺省不标。输出字段（response / webhook payload）的存在性经可选 `presence`（`FieldPresence`：`optional` / `nullable` 两个布尔与 `empty` 字面量三个独立事实，加一条 `condition`（同为 `ConditionText`，多条在唯一的 presence rule 内逐条渲染，条目不绑定到具体事实）；没有独立的 conditional 标志）表达，由 `describeFieldPresence()` 派生为**类型记号而非文字**：字段名后缀 `?`（键可缺失，TS 可选属性惯例）、类型并入 `| null`（OpenAPI 3.1 / GitHub 惯例）、调用方给出的字面空形式作为又一个联合成员（`string | null | ""`、`json<object> | "{}"`），零词汇，唯一文案键 `mayBeOmitted` 就地解释 `?`（TS 记号对非 TS 读者并不自明）：字段行的 `?` 是 `UTooltip` 触发按钮，鼠标悬停与键盘聚焦可读、Escape 关闭，`mayBeOmitted` 同时是按钮的 accessible name 与提示正文——两者同句，所以显式去掉原语注入的 `aria-describedby`，只念一次；成本是**每个带 `presence.optional` 的字段恰好多一个焦点停靠点，其余字段为零**（悬停可见的内容必须键盘可达，WCAG 2.1.1）。`?` 带 `select-none`，选中复制字段名得到的是 `failureCode` 而非 `failureCode?`。按钮自身没有动作，点击不关闭提示（`disable-closing-trigger`）。键盘聚焦时 `?` **自身点亮**为主色块（`bg-primary` + `text-inverted`，悬停为同形的 `bg-elevated`），刻意不用 kit 通用的外扩焦点环：单字符盒子上那个环会变成竖椭圆，外扩量还会压住字段名末字符；填充不越出按钮自身盒子，`name?` 间距不变。强制配色模式会抹掉背景，所以保留一条透明 outline 作兜底。原语忽略触屏指针，触屏不出提示：字段区的统一记号说明由消费页面承载（gallery 的 endpoint / webhook 参考页在字段区上方各放一句可见的记号说明作示范），组件不提供图例 API。依赖应用根部的 `<UApp>`（TooltipProvider，与本 kit 其它 tooltip 同一前提）；记号只说「可能」，有条件行即有条件，不再区分「按条件」措辞；不带 `presence` 即「始终存在」，不从缺少 `required` 推断。详情保持 developer call-time 顺序：deprecated 迁移提示 → conditional rule → presence rule（中性 `border-accented` 左边框，与 conditional rule 同形不同色）→ description → caveat → enum/constraints/examples/lifecycle；普通事实统一为 label/value definition grid，长值可断行，≥2 条 constraint 才升级为带计数表。条件行只用 `inline-start` 琥珀边框、不带 lead-in（summary 的 `CONDITIONAL` 由 `fieldRequiredState` 从 `condition` 派生，词只出现一次），caveat 用 `inline-start` 琥珀边框 + `bg-warning/10` + label lead-in；`SINCE` 恒中性。子字段由 `UCollapsible` 揭示，窄容器收紧缩进；深链接由 `useFieldAnchor` 驱动。字段值是 oneOf/anyOf/allOf 时，`composition` 在子字段后委托 `<SchemaComposition>`。数组元素 / record 成员 / 编码内容是「值的形状」而非字段，经可选 `value`（`FieldValueNode`，无名、不计入子字段数、可递归）表达：身份行保持实际传输类型，简单数组 / 字典由 `fieldTypeSummary` 合并为 `array<T>` / `map<string, T>`；嵌套与解码形状补在正文，值的约束保留各自作用域，下方有真实属性时才出恰好一个折叠区（复用 `showChildren` / `hideChildren`），由内部 `FieldValueStructure` 承担。数据模型与 labels 继续由 `utils/field.ts` 提供，全部 chrome（含递归子行）经单个 `labels` 对象本地化，见「Display-model seam → FieldItem 的通用交互约束 / 值结构」 | — |
...
- `components/SplitPaneHandle.vue`（`<SplitPaneHandle>`）—— 纯展示 + a11y 的分隔把手。默认 `appearance="grip"` 是 1px `border-default` 分隔线 + 居中 grip 药丸；**grip 默认隐藏（`opacity-0`），hover / 拖动中（`dragging`）/ 键盘 `focus-visible` 时才浮现**。特殊外框布局可用 `appearance="edge"`：静息透明的贴边 1px hairline 在 hover / focus / drag 时变成 2px primary，不再重复维护 separator DOM（`SidebarNav` 即复用此形态）。公开 prop 使用领域名 `label`/`controls`/`value`/`min`/`max`，组件统一映射为 `aria-label`/`aria-controls`/`aria-valuenow/min/max`；另含方向键/Home/End/Enter 键盘操作和 `col/row-resize` 光标。**只报告意图（`dragStart`/`step(delta, coarse)`/`jump` 事件），不持有任何数值**；`coarse` 只表达 Shift 意图，具体步长仍由父组件决定。主轴尺寸则由消费方（纵向把手可 `self-stretch` 填满，或传 `sticky h-[calc(...)]` 做视口高钉住）。
  - **坑**：`group-hover/handle:` 在 Tailwind v4 会被包进 `@media (hover:hover)`，所以 grip 的 hover 浮现只在有鼠标的设备上生效（触屏/无头浏览器 `hover:none` 不触发，属预期）；触屏与键盘用户靠 `dragging`（拖动中，非 hover 门控）和 `group-focus-visible/handle`（非 hover 门控）两条路径拿到 affordance。handle 必须使用命名 group，避免被消费布局外层的普通 `group` hover 误触发。别用 `transition-[opacity,background-color]` 这种带逗号的 arbitrary value——逗号会打断 Tailwind 的类名扫描、导致其后同一 `class` 里的工具类不被生成；用普通 `transition` 即可。
...
实现要点：用 ResizeObserver 量两栏内部 `<pre>` 的自然高度（滚动容器封顶时 `<pre>` 仍报告完整内容高）；`RequestExample`/`ResponseExample` 已支持接收 `maxHeight` 预算，非 fill 态下 CodeBlock 先长到内容高再封顶滚动。断点 gate 走 `enabled-from` prop（默认 `lg`），gate 之下回退为堆叠 + 自然高、无把手——**必须与外围 SplitPane 的 gate 保持同一断点**（docs-shell 因外层侧栏挤占宽度而统一用 `xl`），内外层断点不同步会出现「分栏启用但放不下最小宽」或反之的错位。`:style` 绑定**始终返回对象（无值时用空对象 `{}` 占位），绝不 `undefined`**——否则 SSR 水合时 `undefined→对象` 的过渡会被 Vue 跳过、宽/高静默不生效。断点判断用手写 `matchMedia` 监听（`onMounted` 建立），别用 VueUse `useMediaQuery`（此处水合后同步不可靠）。
  ...
  - **坑（务必）：RO 回调里的 `measure()` 必须 `requestAnimationFrame` 延迟，不能同步调用**。虽然 `<pre>` 的内容高不受 budget 影响，但 RO 同时也观察了 pane **包裹层**，而包裹层高度正是 `measure()` 通过 `budgets`→`reqStyle/resStyle` 写入的——同步重测就构成「写高度→同帧再触发观察」的闭环，浏览器会抛 `ResizeObserver loop completed with undelivered notifications`。把回调合并进下一帧即可打断同步投递链，且重分配仍在下一帧内完成、视觉无感——这套「cancel 旧帧、只执行最新、卸载自动清理」的样板已收敛为 foundation 的 `useRafTask`（`foundation-use-raf-task` 切片），CodeRail / SplitPane / EnumTable / SidebarScenarioTags 共用，不要再手写 frame id。同理，`SplitPane` 内部量容器宽算 `max` 时也必须经 `useRafTask` 延迟写入——注意**别用 VueUse 的 `useElementSize`**：它在自己的 RO 回调里同步写 ref，在这种「量尺寸→改 flex→再触发」的场景里照样闭环。直接用 `useResizeObserver` 拿 `contentRect`、在合帧任务里写自己的 `mainSize` ref 才安全。
...
- 正常 description、type、format、shape/codec、count、constraint、SINCE、Default/Example 和图例都属于可读文字。`text-muted` / `text-dimmed` 保留各自信息层级，在明暗两主题的四种中性背景上遵守至少 4.5:1 的完整不透明度契约；计数不叠加 alpha，不能因低强调或 deprecated 而豁免。展开文字保留完整 `text-primary`，用 hover 下划线表达反馈；功能色、到达高亮及其他叠加背景仍须单独验证实际合成对比。
- **配色按含义分配（跨 FieldItem 全体生效）**：
  ...
  - **琥珀="有前提/需留意"，按填充度分级**：同一字段可能同时 conditional + beta + 带 caveat，不糊的办法是**用形态和填充度做阶梯，色相只表达"需留意"**，从轻到重四级：
    ...
    2. 条件行——琥珀 **`inline-start` 边框，无填充、无图标、无 lead-in**。它陈述的是"何时必填"这个**事实**，不是风险，所以是最轻的琥珀物件；边框 + 底色 + 图标三重强调属于重复表达，已移除后两项。多条条件仍是**一条** rule、一条边框：容器内由内部 `ConditionEntries` 渲染，1 条无列表 chrome，≥2 条为 `<ul role="list">`（辅助技术可读出条目数与边界），marker 用中性 `text-muted` 而非琥珀——边框已承载色相，每条再加一个琥珀物件会破坏这个阶梯；组件不推断条目之间的 AND / OR，也不解析句子。**不加 lead-in 标签**：summary 的 `CONDITIONAL` 由这条 `condition` 派生而来（见下），词必然已在屏幕上，30px 内同词重复是噪音；条件句本身（"Required when …"）就是文字通道，琥珀不是唯一通道。
  ...
  - **输出存在性恒中性（`presence`）**：可省略 / 可为 null / 可为空是 payload 形状的**事实**，不是调用方义务，记号（`name?`、`string | null | ""`）与 type 同为 mono 灰阶（`?` 用 `text-toned` 常规字重：它承载「键可缺失」这个事实，层级高于普通元数据、低于字段名；联合并入 `data-field-type` 的 type span），presence rule 走 `border-accented`，**不借用红/琥珀**——必填强度轴只回答"调用方必须做什么"，把响应字段的"可能没有"染成琥珀会让读者把它读成一档必填态。`?` 只会出现在带 `presence.optional` 的输出行；请求行的可选态永远静默、必填出红标，所以 `?` 不会与请求侧混淆。presence rule 与 conditional rule 同形（`inline-start` 边框、无 lead-in、不进 constraints、不进 tooltip——`?` 的 tooltip 只解释记号本身，具体场景条件永远是可见正文），仅边框色区分请求/输出两条轴；presence 的 `condition` 与请求侧同为 `ConditionText`，多条在唯一的 presence rule 内逐条渲染（值节点的 `data-value-presence-condition` 同样），条目**不绑定**到具体事实（可省略 / 可为 null / 可为空）——组件不知道哪条说明哪个记号，这是有意保留的：记号说「可能发生」，句子说「何时」，当前没有需要两者联动的消费者；有 rule 即有条件，记号本身不区分「按条件」（评审结论：条件句就是区分，重复一个措辞是噪音）。`empty` 的字面形式由调用方给出（`""` / `[]` / `{}` / `"[]"`），组件不提供"空"的默认词，也不把不同的空混成一个词；空白字面量视为未声明。type span 不再 `shrink-0`，联合可在 ` | ` 处断行而不溢出窄列。
...
### FieldItem 的值结构约束（`FieldNode.value`）

`children` 只装真实子字段。数组元素、record 成员、编码字符串里的内容没有业务字段名，装进 `children` 只能靠合成 `[]` / 「JSON 字符串内容」假字段——多一层折叠、多一个不该计入的子字段数、约束归属不清。它们是**值的形状**，走可选 `value: FieldValueNode`：`relation`（`item` / `member` / `decoded`）说明它挂在上一层的方式，节点无名、不计数、可递归（`value.value` 表达元素的元素、JSON 套 JSON），`fields` 是值内的真实属性（按字段行渲染，是唯一被计数的东西）。组件仍不解析 schema、不解码字符串、不读 wire path；`relation` / `codec` / `type` 与 `type` / `format` 一样由 adapter 决定。`children` 与 `value` 语义互斥，类型层未强制。值节点的存在性走 `presence: ValuePresence`（`nullable` / `empty` / `condition`，没有 `optional`）。`fieldTypeSummary` 只对显式 `array` + 单层 item、`object` + 单层 member 派生短类型；每层 nullable/empty 保留在该层的联合内，外层 optional 不下传、内层 nullable 不上浮。多层 value 保持字段原始 type，由正文逐层补全，未声明的类型不猜测，任意作者类型字符串不解析或改写。FieldAnnotation 与 FieldItem 共用此派生。
...
1. **折叠动词复用 `showChildren` / `hideChildren`，不新增按边界区分的动词。** 专用 JSON 动词会落在该端点几乎每一行，不区分任何东西；边界由正文的形状说明与保留的值作用域承载。`FieldValueLabels` 不含任何 `show*` / `hide*` 键，测试锁死。
...
3. **字段头展示传输类型，解码说明进入正文。** `string` 仍为 `string`，不与 `json<object>` 交换权重，不增加“传值”副标签。字段原有 `format` 保留。当前匿名区域中每层已声明的 relation / codec / type / nullable / empty 以短说明按顺序拼入正文，即使作者没写 description 也不遗漏；没有真实字段或 composition 的多层数组不在头部堆叠泛型。真实结构边界之后的编码信息仍由值区域按覆盖节点去重，不跨边界移动要求。
...
消费端迁移：`FieldNode.value` 是加法，既有消费者不受影响。adapter 把 `[]` / 「JSON 字符串内容」假字段改为 `value` 节点，`x-onerway-format: json_string` + `contentSchema` 映射为 `{ relation: 'decoded', codec: 'json', type: <解码形状>, fields: [...] }`；可继续传 `FieldNode.format`（既有能力）；解码形状由 value 正文说明承载，不能把 string 改成解码后的 object。
...
4. **单事实密度。** 尚未被上述简单事实整合覆盖的一个块，只陈述一件事时渲染为一行 `LABEL │ value`，两件以上升级为标题 + 列表；由 `describeValueRequirements` 判定，模板只按种类渲染、不读 `constraints[0]`。`ValueRequirementsBlock.compact` 是可辨识联合：`{ kind: 'constraint', text }`（恰好一条无 label constraint，且无其它任何事实）、`{ kind: 'presence', expression }`（只有 presence 记号、条件句为空、无 constraint / description / enum / example / default / caveat；`expression` 即 `presenceTypeExpression(node.type, unionTail)`）、`null`（标题形态）。带条件句的 presence 因为要渲染 rule，不进紧凑行。字段自身的约束带（`FieldItem` 单条约束行 vs `CONSTRAINTS (N)` 表）已遵循同一密度，判定仍在模板里，不在本次范围。紧凑行无损：只把已存在的记号换位置，不省略作者输入。两种紧凑行样式分列：constraint 紧凑行保持正文样式与 `InlineMarkdown`；presence 紧凑行用 mono 灰阶记号 + `translate="no"`，落在 `dd[data-value-presence]`，与标题形态的 `[data-value-presence] p` 同名可查；紧凑行的 `dl` 带 `data-compact="constraint" | "presence"`（标题形态无此属性），供测试与消费者辨别密度。紧凑行允许自然换行，不扩展为所有内容块都必须单行。 标题形态的 scope label 与短 presence 记号同行，窄容器自然换行；条件句保持独立整行，多事实列表不压缩为 compact。children / value 折叠触发器用 `hover:bg-elevated` 提供悬停反馈，保留文字、chevron 与 focus-visible 轮廓，不使用链接式下划线。
...
按此推导：gallery `refunds` 的简单元素类型并入字段标题，元素条件保留明确作用域；`extra` 的解码类型随描述展示，不再额外生成「值要求」标题；`txnOrderMsg → products` 深层嵌套每个真实边界恰好一条线。未被整合的复杂值说明仍遵循上述 compact 与标题分支。

**历史 V3 裁定依据（本轮未将旧数据视为重新验证）**（2026-09-21，gallery `txnOrderMsg → products`，decoded → item → decoded → item + 子字段）：桌面（字段容器 960px）每层 17px（1px 线 + 16px），行宽 960 → 943 → 926；320px 视口每层 13px（1px + 12px），行宽 288 → 275 → 262，identity 剩余 226px，无横向溢出；375px 视口 343 → 330 → 317。迁移前页面级 `SchemaComposition` 的嵌套 composition 固定 16px，与字段内 320px 下的 12px 不一致；迁移后三种上下文统一按容器查询收紧且都可读，因此不做容器查询之外的缩进机制。

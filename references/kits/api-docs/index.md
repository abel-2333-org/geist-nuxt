...
- **配色按含义分配（跨 FieldItem 全体生效）**：
  ...
  - **琥珀="有前提/需留意"，按填充度分级**：同一字段可能同时 conditional + beta + 带 caveat，不糊的办法是**用形态和填充度做阶梯，色相只表达"需留意"**，从轻到重四级：
    ...
    2. 条件行——琥珀**左边框，无填充、无图标、无 lead-in**。它陈述的是"何时必填"这个**事实**，不是风险，所以是最轻的琥珀物件；边框 + 底色 + 图标三重强调属于重复表达，已移除后两项。**不加 lead-in 标签**：summary 的 `CONDITIONAL` 由这条 `condition` 派生而来（见下），词必然已在屏幕上，30px 内同词重复是噪音；条件句本身（"Required when …"）就是文字通道，琥珀不是唯一通道。

...
- **全站全文搜索也在这一侧**：`@nuxt/content` 就位后，用 Nuxt UI 的 `<UContentSearch>` / `<UContentSearchButton>`（`⌘K` 模态、跨整站文档正文）。它绑 content、属消费项目职责，**不进 kit 切片**。正确的落位是 **app 顶栏 / navbar**（参考 Nuxt UI / Vercel 文档站），与 `<ApiDocsSidebarNav>` 侧栏内的「就地过滤搜索 + method chips」分属**不同层级**——两者各司其职、靠层级区分。**不要**把 `UContentSearchButton` 塞进侧栏顶部（会和就地过滤框变成两个雷同搜索框上下紧贴），更不要把 `UContentSearch` 焊进导航组件。

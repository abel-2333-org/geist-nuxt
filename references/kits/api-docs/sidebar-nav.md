...
- **全局搜索是唯一搜索入口、同时匹配用途名与场景标签**：顶部一个 `UInput` 过滤所有板块——板块标题命中则整块保留，否则只留 **label 或场景标签命中**查询的 item（`matchesText` 同时查 `label` 与 `scenarios`）；有查询时**命中板块强制展开**，让结果始终可见，计数徽章显示 `命中/总数`。因此**输入场景名（如「订阅」）会浮出所有服务该场景的接口**，跨板块聚合了这个多对多关系；「大板块子项多」的检索需求也由这个全局搜索覆盖，无需每块再放搜索框，也不需要额外的过滤 chips。
...
- **场景标签后置、用途名前置对齐**：用途名 leading（`flex-1 truncate`），场景标签 trailing（`shrink-0` + `gap-1` 并排）。后置的好处是不论一个接口挂几个场景，各行用途名的起始 x 都对齐；标签数量在右侧自然伸缩，不挤压标签。
...
- 搜索：empty / has-query（命中板块强制展开 + 计数转 `命中/总数`，空分组整组隐藏、只留有命中的领地）/ no-results（空态文案）。has-filter 即 has-query（无独立的 method 过滤态）。
- 场景标签：静态展示，无交互态（不可点选、不参与过滤）——它只是标注该接口服务哪些场景，检索走顶部搜索。
...
- 场景标签是中性 `UBadge`（非交互），文字（订阅/授权…）本身即可访问名，不靠颜色传递信息；作为接口链接可访问名的补充，随 `ULink` 一起被读出。
      ...
      sections: [
        {
          ...
          items: [
            // 用途名（非路径）；一个接口服务多个场景，只出现一次，用 scenarios 标注
            { label: '创建结算会话', to: '/api/checkout-create', scenarios: ['支付', '订阅'] },
            { label: '结算会话', to: '/api/checkout-session', scenarios: ['支付'] },
          ],
        },
      ],
    },
...
> 不需要分组标题时可退回 `:sections="[...]"`（扁平列表，自动包成一个无标题组）；`kind` 仍可逐板块指定。接口的 `scenarios` 是可选的——不给就不显示场景标签，行退化为纯用途名链接。

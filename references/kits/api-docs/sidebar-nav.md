...
- 方法色标（`HttpMethodBadge`）文字即动词（GET/POST…），颜色只是**强化**、非唯一信号；场景标签文字（批处理/实时查询…）本身即可访问名，`sr-only` 再兜底全量场景，折叠项的 popover 触发器有 `aria-label`（文案由 `scenarioOverflowLabel` 注入，默认 `View all N scenarios`）。

...
- `<ApiDocsScenarioTags>` — 本 kit 的兄弟切片，接口行后置的场景标签簇（测量式溢出 + `+N`/计数 chip 降级）封装于此，内部由中性 `UBadge` 构成；随本切片一起拉入。
- `<UCollapsible>` / `<ULink>` / `<UInput>` / `<UBadge>` / `<UKbd>` / `<UTooltip>` — Nuxt UI 原语。

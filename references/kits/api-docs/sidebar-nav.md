...
> 真源在 `kits/api-docs/components/SidebarNav.vue`；根 registry target 保留 `app/components/api-docs/SidebarNav.vue`，所以模板名是 `<ApiDocsSidebarNav>`。数据无关：导航数据模型内联随组件走，所有 chrome 文案经 props 注入（i18n-ready）。
...
- `<UContentSearch>` / `<UContentSearchButton>` — Nuxt UI 的全站全文搜索（`⌘K` 模态，绑 `@nuxt/content`）。**不在本切片内**，由消费项目在 app 顶栏接入；接线见 `project-setup.md`。

...
会换行的组件必须**显式声明哪些部位不可拆分**，并且用**关系**语言而非**位置**语言写：
...
位置语言唯一的落地方式是像素阈值，而阈值在内容可变时必然错——**只要契约里出现「在某宽度下…」而那个宽度其实由内容决定，就是写错了**。关系语言直接映射到 DOM 结构（一个 `flex-nowrap` 容器 + 一个 `data-*` 标记），既没有可写错的阈值，也让测试有稳定的断言对象。

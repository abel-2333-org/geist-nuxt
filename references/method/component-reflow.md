...
上面那串固定宽度只探**端点**。而回流缺陷恰恰活在两个端点之间的**带**里，且带的位置由内容决定——任何固定宽度清单都点不到它。`OperationTarget` 的孤立复制键（#68）就是这样躲过了完整 gate：全绿，同时 bug 一直在。

对每个声明了回流契约的组件，按**组件容器**宽度（不是 viewport）连续扫描，而不是抽查：

1. 取至少 3 组 fixture：短内容、真实内容、长内容（长 host / 长 path / 长标签各来一条）；
2. 从 320px 逐像素扫到组件的自然单行宽度以上；
3. 记录状态**序列**，断言它只包含契约允许的状态、且状态转换次数等于契约声明的次数；
4. 同时断言全程 `scrollWidth <= clientWidth`（无页面级横向溢出）。

多出来的状态就是缺陷；转换点在不同 fixture 上漂移是正常的（内容驱动），**转换次数变化才是回归**。

对应的单测形态：断言**结构不变量**，不要断言断点类名。

```ts
// ✅ 锁契约：单元存在、且不可拆
expect(unit.classes()).toEqual(expect.arrayContaining(['flex', 'flex-nowrap', 'min-w-0']))

// ❌ 锁实现：断点类名一改就红，而且它全绿时 bug 可以照样存在
expect(segment.classes()).toContain('@md/target:flex-[0_3_auto]')
```

> 扫描目前是人工 / 一次性脚本；通用 gallery 宽度矩阵 story 与自动化 check 是已知后续事项。

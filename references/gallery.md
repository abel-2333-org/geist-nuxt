...
## 条目结构：GalleryItem + GalleryVariant

一个组件 = 一个 `<GalleryItem>`，内含若干 `<GalleryVariant>`：
...
>
  <GalleryVariant label="Variants">
    <UButton>Primary</UButton>
    <UButton variant="outline">Outline</UButton>
  </GalleryVariant>
  <GalleryVariant label="Sizes">
...
- **GalleryItem**：卡片外壳。`name`（等宽字体，如组件名）+ `description`（一句话）+
  可选 `usageHref`（右上角 Usage 链接，指向 references 或 Nuxt UI 文档）。
- **GalleryVariant**：一个带标签的实例块。`label` 是小号大写标签（Variants / Sizes /
  States / With icon…），下面摆该变体的**真实组件实例**。
...
2. 加一个 `<GalleryItem>`，填 name / description / usageHref。
3. 按变体维度（变体、尺寸、状态、带图标…）拆成多个 `<GalleryVariant>`，每块摆真实实例。
...
- core 组件的默认预览展示走 core 里的 `ShowcaseComponents`（轻量一览），
  与 gallery 的 GalleryItem（可摆更多、带变体标签）是同物种、不同繁简度，各司其职。

...
`<DemoApiDocsCodeRail>` 负责 Request / Response 的纵向分配与内容优先重分配，gallery 内有 reference 页与 `DocsShellReference` 两个消费者。保持私有的依据不是消费者数量，而是它与页面 recipe 的 chrome（sticky 偏移、视口高长条、断点 gate）强耦合、没有稳定的跨项目分发契约（判断标准见 ADR-004），因此：

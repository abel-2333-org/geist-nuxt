...
本 skill 的**真源仓库**是 GitHub `Abel-Wang777/geist-nuxt`（`main` 分支）。v0 记忆区里的这份 skill 是**下游镜像**——任何在记忆区里的改动，**必须**对账同步回 GitHub，否则真源会漂移。
...
3. **GitHub `Abel-Wang777/geist-nuxt` `main`** — 真源。
...
1. 取 base：`gh api repos/Abel-Wang777/geist-nuxt/git/refs/heads/main --jq '.object.sha'`，再取其 tree。
...
1. 拉完整递归 tree：`gh api "repos/Abel-Wang777/geist-nuxt/git/trees/main?recursive=1"`（确认 `.truncated==false`）。
...
- 拉远端 tree：`gh api "repos/Abel-Wang777/geist-nuxt/git/trees/main?recursive=1"`。

...
> 上表是**任何项目都可用的通用组件**（Nuxt UI 原语）。领域专属组件（如 API 文档场景的 EndpointHeader / ParamsTable / CodeSample / ResponseBlock）不在此处，见 `kits/`（用到才加载）。「主要组件」列即官方组件名映射，`U` 前缀为 Nuxt UI 自动导入名。

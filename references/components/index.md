...
> **可拖动分栏（左右 / 上下 resizable split）**：Nuxt UI 只有横向 %-based 的 `useResizable`（服务于 Dashboard 侧栏），没有通用的双轴分隔原语。首选 foundation 的 **`<SplitPane>`**——声明式、自包含、MDC/Nuxt Content 友好的双 slot 容器（`#start`/`#end`），把拖动/持久化/断点/键盘/SSR 全封装，一行即可复用。它内部由 `<SplitPaneHandle>`（把手，纯展示+a11y）+ `useSplitPane`（轴无关拖动状态 + cookie 持久化）+ `useBreakpointGate`（SSR 安全断点门控：服务端恒 stacked、挂载后翻转、跟随 `enabledFrom` 运行时变化；`'always'` 不设门）+ `useRafTask`（合帧调度：把 ResizeObserver 等高频通知延迟到下一帧、只执行最新一次，避免同步布局反馈环；无 rAF 环境同步执行，scope dispose 自动取消）组成，需要更特殊布局时才直接碰这几个零件。api-docs 的 `<CodeRail>` 复用同一 `useBreakpointGate` 与 `useRafTask`，`<EnumTable>` 与 SidebarNav 的场景标签也用 `useRafTask` 合并测量——门控与合帧调度都不再各自维护镜像实现。内容优先重分配不进 foundation——它由 api-docs kit 的 `<CodeRail>` 承载（kit 领域组件，随 registry 分发，见 ADR-010），详见 `kits/api-docs/index.md` 的「可拖动分栏」一节。

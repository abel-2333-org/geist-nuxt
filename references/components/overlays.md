...
**props**：`id`（glossary 键）`entry`（直接传 `GlossaryEntry`，绕过 glossary 供一次性行内概念；给定时 `id` 被忽略）`labels`（继承 `AnnotationPopoverLabels`，含 `category`/`learnMore`）。默认 slot 可覆盖可见短语（缺省用 glossary `term`）。术语库经 `useGlossary` 的 `provideGlossary()` 由页面/布局按需注入，组件侧 `useGlossary()` 读取。

...
行内 markdown 子集渲染器（foundation）：同步、SSR 稳定、零依赖。支持 `` `code` ``、`[链接](url)`、`**粗**` / `__粗__`、`*斜*` / `_斜_`、`***粗斜***` / `___粗斜___`、`~~删除~~`，全部映射到设计系统的 Prose 组件；解析递归可嵌套。**props**：`text`(string)。仅面向"永远是行内"的作者文案——比 MDC 轻、且不会在一页多实例时触发 SSR→client hydration 失配；若真需要块级 markdown 再考虑 MDC。

**不吃技术文本**（API 文案的核心安全属性）：标记要么整段被识别、要么整段按字面渲染，绝不半吞后在页面上留下裸露的 `*` / `_`。同一标记字符按最长游程优先匹配（`***` → `**` → `*`），`_` 家族两侧要求非词边界且 `_` 自身计入该边界——因此 `snake_case`、`MAX__VALUE__LIMIT`、`[a-z_]__[0-9]`、`1_000_000`、`2*3*4` 都原样输出。**没有反斜杠转义**（`\*`）：API 约束文案里常带正则和通配（`^\d{3}\*$`、`C:\*.txt`），转义规则会把这些反斜杠悄悄吃掉；要字面标记就用 code span，其内容永不二次解析。

已知边界：内层标记与外层收尾标记**紧贴**时（`**粗 *斜***`）仍会残留一个标记——彻底解决需要 CommonMark 的 delimiter-run 算法，当前有序规则表做不到；中间隔任意文本（`**粗 *斜* 尾**`）则正常嵌套。

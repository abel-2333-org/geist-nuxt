...
- [ ] `ApiDocsSidebarNav` 内含一个通用的「可折叠分组 + 搜索过滤 + 拖拽/键盘调宽」侧栏外壳，评估把该外壳上提 core（如 `<NavSidebar>`，用 slot 交出每行渲染）→ **暂不晋升，整体留在 kit**。按 `method/component-reflow.md` 第 0 步自检：①通用性不足——item 模型焊死 `method`/`scenarios`/`endpoints` 这套 API 文档概念，且依赖 `ApiDocsMethodBadge`/`ApiDocsScenarioTags` 两个兄弟切片，整体是 api-docs 业务装配而非通用件；②纯外壳部分确实"无现成"（Nuxt UI 的 `UNavigationMenu`/`UContentNavigation`/`UTree` 均不含"可搜索+可折叠+可调宽的侧栏骨架"）。延后理由：目前仅 api-docs 一个消费者，现在抽象通用外壳属提前抽象（YAGNI），真实形状验证不足。触发信号：出现第二个需要同款侧栏骨架的场景（届时按第 5 节「跨 kit 共性上提 core」执行——分层拆分而非整体平移：外壳提 core、method/场景标签留 kit 用 slot 填入）。

...
铁律：**kit 组件只认 ViewModel，不认识任何私有字段**——像「文案无关」��样做到「数据格式无关」。
...
- [ ] `packages/kits/api-docs/app/components/api-docs/Section.vue`（registry:block）把 demo 数据（requestSamples/responseScenarios）硬编码在 kit 包内，违反「数据不进 kit」→ 待重构为数据无关（props 受数据）+ demo/fixture 迁入 gallery，见 `references/gallery.md`「Story 数据分层」

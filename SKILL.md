...
- **`@geist-nuxt/core`（npm 公共包，Nuxt layer）** = 基座：token/紫色 ramp/Geist 覆盖的全局 CSS、跨场景公共组件（`CopyButton`、`SplitPane`、`ThemeToggle`）、**自描述展示页 `GeistShowcase`**（含 `ShowcaseHero/Foundations/Components` 子件）、composables（`useCopy`、`useSplitPane`）。项目 `extends: ['@geist-nuxt/core']` 即自动获得全部。
...
provider（`app/app.vue` 里的 `UApp`）、全局 CSS、字体、明暗切换都已由 core layer 接好。**新增通用组件要回流真源 `packages/core` 并在 `ShowcaseComponents` 加展示条目**（人工采纳关口），发版后所有会话预览自动带上。
...
2. **按复杂度决定要不要写规格**（`references/method/component-spec-template.md`）：
   - **���交互 / 状态 / 焦点管理的组件**（可展开、可复制、可切换、带校验等，如 `CodeBlock`、`CopyButton`）→ **走全套**：anatomy → state model → accessibility 三张表都过一遍再实现。这是 geist-nuxt 方法论的核心价值所��。
...
6. **回流进 core（要让它成为系统组件、进默认预览时才走）**：按 `references/method/component-reflow.md` 的 checklist——搬进 `packages/core` → 加 `ShowcaseComponents` 条目 → bump core minor 版本 → bump starter 精确依赖 → CI 发版。**人只拍「该不该进」和「授权推送」两个决策，其余 AI 一条龙。**
...
| 场景组合 compositions | `compositions/*.md`（按抽象层级分） | 招牌场景→`ShowcaseComponents`；整页→starter/gallery 应用本身 | 文字✅ / 可视❌ |
...
| 一个场景组合 | `compositions/<层级>.md` | 够招牌→`ShowcaseComponents`；整页级→starter/gallery 应用本身，**不强求逐条镜像** |
...
- **整屏装配（页面外壳、区块、卡片网格、表单、空/加载态、a11y 清单，含完整外壳示例）**：`references/compositions.md`（后续将按抽象层级拆入 `references/compositions/`）

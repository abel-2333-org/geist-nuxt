...
3. **实现**：`<script setup lang="ts">`，只用 `@nuxt/ui` 原语 + 语义 token 拼装，守住上面的硬规则（focus 环、aria-label、`UFormField`、无固定宽度）。注意自动导入的子目录前缀（见 `conventions.md`）。**组件文案要「无关化」——���过 props/slot 传入，不硬编码字符串**（多语言接线见下方 i18n 说明）。
